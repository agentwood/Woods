...
import { sendAuthMail } from "./send-mail";
...
export const authConfigured = !authDisabled && (useGrokBroker || useGoogle || emailAndPasswordEnabled);
...
export const auth = betterAuth({
  ...
  trustedOrigins: async (request?: Request) => {
    ...
    try {
      ...
      if (host === "joinwoods.co" || host.endsWith(".joinwoods.co") || host.endsWith(".netlify.app")) {
    ...
    ? {
        emailAndPassword: {
          ...
          sendResetPassword: async ({ user, url }: { user: { email: string }; url: string }) => {
            await sendAuthMail({
              to: user.email,
              subject: "Reset your Woods password",
              url,
            });
          },
        },
